...
def func_baby(RATE, start_index, x, speed_factor=2.0, freq=200):
    # speed
    x_arr = np.asarray(x)
    num = len(x_arr)
...
def add_sound(x, block_ind, wf, wave_length, wave_width, gain):
    #x_arr = np.asarray(x)
    L = len(x)
    ...
    y = x + np.round(gain*sound_portion_arr/100).astype(int)
